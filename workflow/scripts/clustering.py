...
def get_dendrogram(df, linkage, thresh=0, Plot=False, title="Hierarchical Clustering Dendrogram", figsize=(50, 20)):
    if Plot:
...
def hierarchical_clustering(heatmap_data, distance_threshold):
    ...
    g = sns.clustermap(
        ...
        col_colors=row_colors_cl,
    )  # , vmin=vmin2, vmax=vmax2, method=distmethod)
...
if __name__ == "__main__":
    ...
    for lib in lib_names:
        ...
        depth_df = append_GO_clusters_to_depth_df(
            sim_matrix_cache_folder, depth_df, figpath, lib, project_name, max_thresh=go_sem_sim_max_distance
        )
